...
function Index() {
  return <div className="bg-ink">
    ...
    <div className="overflow-hidden border-y border-primary/30 bg-primary py-3 text-primary-foreground"><div className="flex w-max animate-ticker gap-12 editorial-label">{[0,1].flatMap((copy) => ["Lahore specialists", "75+ properties sold", "5.0 Google rating", "Open 24 hours"]).map((item,index) => <span key={`${copy}-${index}`} className="flex items-center gap-12">{item}<span>◆</span></span>)}</div></div>
    ...
    <section className="border-t border-border px-5 py-24 sm:px-8 lg:px-12"><Reveal className="mx-auto flex max-w-[1500px] flex-col items-start justify-between gap-8 sm:flex-row sm:items-end"><div><p className="editorial-label text-primary">Private consultation</p><h2 className="mt-5 max-w-3xl text-5xl leading-none text-cream sm:text-7xl">Your next address begins with a conversation.</h2></div><Button asChild variant="editorial" size="lg"><Link to="/contact">Speak with us <ArrowUpRight /></Link></Button></Reveal></section>
  </div>;
}
